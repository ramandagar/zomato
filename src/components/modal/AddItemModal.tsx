...
const AddItemModal: FC<{ onClose: () => void, item: any, restaurant: any }> = ({ onClose, item, restaurant }) => {
    ...
    item.customizationOptions.forEach((customization: any) => {
      if (customization?.required) {
        ...
        if (defaultOptionIndex !== -1) {
          defaultSelectedOptions[customization.type] = defaultOptionIndex
          initialPrice += customization?.options[defaultOptionIndex]?.price || 0
        }
      }
    })
    setData((prev) => ({ ...prev, selectedOptions: defaultSelectedOptions, price: initialPrice }))
  }, [])
    ...
    <View>
      ...
      <View style={styles.footerContainer}>
        ...
        <TouchableOpacity style={styles.addButtonContainer} onPress={addItemToCart}>
          <CustomText color='white' fontFamily='Okra-Medium' fontSize={12} variant='h5' style={{}}>Add item - Rs {data.price}</CustomText>
        </TouchableOpacity>
        <SafeAreaView />
      </View>
    </View>
